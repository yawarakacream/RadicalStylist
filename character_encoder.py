...
if encoder_type == 0:
    ...
    class CharacterEncoder(nn.Module):
        def __init__(
            ...
            posemb_precision: int = 256,
        ):
            super(CharacterEncoder, self).__init__()
            
            self.embedding_dim = embedding_dim
            self.len_radicals_of_char = len_radicals_of_char
            self.posemb_precision = posemb_precision

            assert self.embedding_dim % 2 == 0
            self.rademb_dim = embedding_dim // 2
            self.posemb_dim = embedding_dim // 2
            ...
            self.precomputed_posemb: list[list[float]] = []
            for p in torch.linspace(0, 1, self.posemb_precision):
                T = 1000
                self.precomputed_posemb.append([])
                for i in range(0, self.posemb_dim // 4, 2):
                    theta = p / (T ** (i / self.posemb_dim // 4))
                    self.precomputed_posemb[-1].append(math.sin(theta))
                    self.precomputed_posemb[-1].append(math.cos(theta))
            ...
            for char in batch_char:
                ...
                for radical in char.radicals:
                    embedding.append(
                        ...
                        + self.precomputed_posemb[int(radical.width * self.posemb_precision)]
                        + self.precomputed_posemb[int(radical.height * self.posemb_precision)]
                    )
                embedding += [[0] * self.posemb_dim] * (self.len_radicals_of_char - len(embedding))
